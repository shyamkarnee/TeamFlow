...
export const Login: React.FC = () => {
  const { signInWithGoogle, signInWithEmail, signUpWithEmail, signInWithDemo, loading } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (message.includes("auth/unauthorized-domain") || message.includes("unauthorized-domain")) {
        setErrorMsg(
          "Google Sign-In failed because this development domain is not authorized in the Firebase console. Please use the 'Demo Accounts' tab or 'Email Account' tab to sign in instantly!"
        );
      } else {
        setErrorMsg("Failed to sign in with Google. Please use a Demo Account.");
      }
    }
  };
        ...
        <div className="p-8">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1 py-2 px-1 text-xs font-semibold rounded-lg transition-all ${activeTab === 'google' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
            >
              Google Login
            </button>
          </div>
          ...
          {activeTab === 'google' && (
            <div className="space-y-4">
              <p className="text-xs text-slate-500 text-center mb-4">
                Federated single sign-on with your Google account. Requires domain verification setup.
              </p>
              ...
              <button
                ...
                disabled={isGlobalLoading}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-slate-900 text-white rounded-xl text-xs font-semibold shadow-sm hover:bg-slate-800 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isGlobalLoading ? (
